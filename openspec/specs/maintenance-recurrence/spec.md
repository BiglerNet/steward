# maintenance-recurrence Specification

## Purpose
Backend computation of last-done/next-due status per `(TemplateStep, Engine)` pair on an asset, and the per-asset maintenance schedule endpoint that exposes it. Purely a read-only layer over existing `ChecklistItem`/`TemplateStep`/`EngineHoursLog`/`MileageLog` data — no scheduler, no new mutable state.
