# frontend-maintenance-kanban Specification

## Purpose
The household-wide kanban board for `MaintenanceItem`s across all of a household's assets, backed by the `maintenance-items` capability's household-wide list endpoint, with drag-and-drop status changes.
