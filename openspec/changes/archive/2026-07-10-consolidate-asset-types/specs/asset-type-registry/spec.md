...
- **THEN** HTTP 200 is returned with one entry per `AssetCategory` value, each including category, group, structural type, display label, default usage tracking mode, typically-has-engine, VIN decode support, typical permit kinds, applicable fields, and icon color
